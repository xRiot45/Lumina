...
export class OrdersService {
    ...
    async updateOrderStatus(orderId: string, dto: UpdateOrderStatusDto): Promise<{ success: boolean }> {
        this.logger.log({ message: 'Initiating order status update', orderId }, this.context);
        ...
        try {
            ...
            const response = await firstValueFrom(
                this.ordersClient.send<{ success: boolean }>({ cmd: 'update_order_status' }, payload),
            );
    ...
    async confirmOrder(userId: string, orderId: string): Promise<{ success: boolean }> {
        ...
        try {
            ...
            const response = await firstValueFrom(
                this.ordersClient.send<{ success: boolean }>({ cmd: 'confirm_order' }, payload),
            );
            return response;
        } catch (error: unknown) {
            this.logger.error(`[Gateway] Raw Error from Carts Microservice: ${JSON.stringify(error)}`);
    ...
    async findAllMyOrders(userId: string, query: OrderPaginationDto): Promise<IPaginatedResponse<OrderResponseDto>> {
        this.logger.log(`Fetch all orders for userId: ${userId}`, this.context);
        ...
        try {
            ...
            const response = await firstValueFrom(this.ordersClient.send({ cmd: 'find_all_my_orders' }, payload));
            return response;
        } catch (error) {
            this.logger.error(`[Gateway] Raw Error from Microservice: ${JSON.stringify(error)}`);
